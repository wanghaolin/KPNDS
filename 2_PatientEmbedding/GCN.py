'''
-GCN to obtain 128-dimensional patient embedding
-Randomize initial features as patient node features
'''
import torch
...
def convert_to_pyg_data(G):
    ...
    num_features = 128
    node_features = torch.rand((num_nodes, num_features), dtype=torch.float)  # Randomly initialize the feature matrix
...
def add_embeddings_to_df(original_df, embeddings_tensor, patients):
    ...
    original_df['embedding'] = original_df['PatientID'].apply(
...
df.to_csv('ADR-embedding-GCN.csv', encoding='gb2312', index=False)
